...
const HourlyChart = ({ accountId }) => {
  const [type, setType] = useState("win");
  const {data:hourlyData, status} = useQuery({
    queryKey: ["hourlychart", accountId, type],
    queryFn: async() => {
      const queryResult = await fetch(`${MY_API_URL}/accounts/analysis/${accountId}/hours`)
      .then(async res => {
  ...
  return (
    <div className="relative">
      ...
      <BarChart chartData={hourlyData} key={type}/>
    </div>
  )
}
